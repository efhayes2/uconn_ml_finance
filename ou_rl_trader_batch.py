...
class RLParameters:
    ...
    replay_buffer_size: int = 100000 # New: Maximum size of the replay buffer
    batch_size: int = 64         # New: Number of experiences to sample for each learning update
    learn_every_steps: int = 4   # New: How often to perform a learning step (e.g., every 4 steps)
    learn_start_size: int = 1000 # New: Start learning only after 1000 experiences
...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Convert batch of experience tuples into separate numpy arrays for vectorized processing
